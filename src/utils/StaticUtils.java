...
public class StaticUtils {
    ...
    /**
     * 
     * @param data block to pad
     * @param blockLength length of the block after padding
     * @return data with padding
     */
    public static byte[] pkcs7(byte[] data, int blockLength) {
	if(data.length >= blockLength)
	    return data;
	
	int padAmount = blockLength - data.length;
	byte[] paddedData = new byte[blockLength];
	for(int i = 0; i < data.length; i++) {
	    paddedData[i] = data[i];
 	}
	
	for(int i = 0; i < padAmount; i++) {
	    paddedData[data.length + i] = (byte) padAmount;
	}
